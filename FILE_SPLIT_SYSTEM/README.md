...
状态：manual splitter 1.4.1 已实现但尚未通过 Illustrator 实机验证；其真实验证状态以 `executors/manifest.json` 为准。
...
1.4.1 的文字转曲门禁使用“临时副本轮廓与原文字轮廓”比较；串联文字或复制不稳定时使用明确记录的安全备用校验。执行器还会在建组前处理未归属对象、按下一个同级章节标题截断混排删除区间，并在成功或失败后只关闭自己打开的工作副本。
