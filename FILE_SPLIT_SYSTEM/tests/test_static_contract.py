...
class StaticContractTests(unittest.TestCase):
    def test_all_json_versions_and_settings_parse(self):
        ...
        self.assertEqual(settings["schema_version"], "1.4.1")
        self.assertEqual(settings["rule_version"], "1.4.1")
        ...
        for path in ROOT.rglob("*.json"):
            ...
            if "schema_version" in payload:
                self.assertEqual(payload["schema_version"], "1.4.1", str(path))
    ...
    def test_manifest_blocks_fast_production_until_real_validation(self):
        ...
        self.assertEqual(manifest["executors"][0]["implementation_status"], "implemented_unverified")
        self.assertEqual(manifest["executors"][0]["validation_status"], "implemented_unverified")
        self.assertEqual(manifest["executors"][1]["validation_status"], "implemented_unverified")
    ...
    def test_jsx_uses_extend_script_compatible_subset_and_forbids_whole_source_pagination(self):
        ...
        self.assertIn("snapshotOriginalCandidates", manual)
        self.assertIn("createOutline()", sources)
